...
def clean_channel_names(raw):
    # EEGBCI EDF'lerde bazen kanal adlarında nokta/boşluk gibi şeyler olabiliyor.
    # standard_1020 ile eşleşmeyi artırmak için sadeleştiriyoruz.
    mapping = {}
    for ch in raw.ch_names:
        new = ch.strip()
        # EDF'de "Fp1." gibi bir şey gelirse noktayı kaldır
        if new.endswith("."):
            new = new[:-1]
        mapping[ch] = new
...
def apply_standard_montage(raw):
    ...
    # on_missing='ignore' -> eşleşmeyen kanallar varsa hata verme
